var AstroTetherBundle = (() => {
  ...
  function createRockSilhouette(rng, radius, sides = 9) {
  ...
  var DRAW_THRESHOLD = 15;
  var WALL_THICKNESS = 12;
  var BOUNCE_ANGLE_THRESHOLD = Math.cos(Math.PI * 0.38);
  var WALL_CONTACT_PUSH = 0.8;
  var WALL_SLIDE_DAMPING = 0.999;
  var WALL_NEAR_DISTANCE = 18;
  var WALL_ADHESION_RELEASE_DISTANCE = 24;
  var PROBE_AIR_DRAG = 0;
  var GRAVITY_FORCE_SCALE = 86e4;
  ...
  var AstroTetherGame = class {
    constructor(canvas2, ui2) {
      this.canvas = canvas2;
      ...
      this.wallContacts = /* @__PURE__ */ new Map();
      ...
      this.probeTrail = [];
      this.planetGravityPulse = /* @__PURE__ */ new Map();
      this.lineTypeEls = {
        adhesion: null,
        bounce: null
      };
    ...
    createProceduralLevel(level) {
      ...
      const pathForces = pathNormal.map((normal, index) => {
        ...
        for (let i = 0; i < planetCount; i += 1) {
          ...
          const dx2 = px - point.x;
          const dy2 = py - point.y;
          const r = Math.hypot(dx2, dy2) || 1;
          const strength = rng.range(45e-6, 9e-5) * (1 + i * 0.12);
          const falloff = 1 / Math.max(r * r, 1600);
          const force = strength * falloff * (planetRadius * 13e4);
          forceX += dx2 / r * force;
          forceY += dy2 / r * force;
        }
      ...
      const computePressSide = (index) => {
        ...
        if (Math.abs(push) < 3e-4) return push >= 0 ? 1 : -1;
        return push > 0 ? 1 : -1;
      };
      const makePlanet = (index, side) => {
        ...
        const candidate = {
          ...
          gravityStrength: rng.range(45e-6, 9e-5),
    ...
    applyPlanetGravity() {
      ...
      for (const planet of this.planets) {
        ...
        const force = planet.gravityStrength * influence * falloff * planet.gravityRadius * GRAVITY_FORCE_SCALE * radiusScale * this.gameConfig.planetGravityForceMultiplier;
        ...
        Body.applyForce(this.probe, this.probe.position, {
          x: radial.x * pull - tangent.x * tangentSpeed * 65e-5 * swirl,
          y: radial.y * pull - tangent.y * tangentSpeed * 65e-5 * swirl
        });
        if (speed > 0.01) {
          const bend = clamp(swirl * 18e-7, 0, 9e-4);
          Body.setVelocity(this.probe, {
    ...
    handleCollisionStart(event) {
      ...
      for (const pair of event.pairs) {
        ...
        if (other.gameType === "speedRing" && this.state === "launched") {
          const velocity = this.probe.velocity;
          const dir = normalize(
            Math.hypot(velocity.x, velocity.y) < 0.2 ? { x: this.goal.x - this.start.x, y: this.goal.y - this.start.y } : velocity
          );
          Body.applyForce(this.probe, this.probe.position, {
    ...
    resolveLineContact(lineBody) {
      ...
      if (this.railCooldown && stroke && this.railCooldown.strokeId === stroke.id && this.railCooldown.until > performance.now()) {
        return;
      }
      ...
      if (lineBody.lineType === "adhesion" && stroke) {
        const direction2 = Math.sign(vDotT) || previousDirection;
        this.beginAdhesionRail(stroke, lineBody, direction2, vDotT, speed);
    ...
    resolveWallProximity() {
      ...
      const absNormal = Math.abs(vDotN) / Math.max(speed, 1e-4);
    ...
    advanceAdhesionRail(dt) {
      ...
      const endReached = (this.activeRail.direction || 1) > 0 ? nextProgress >= stroke.totalLength : nextProgress <= 0;
      if (endReached) {
    ...
    clearActiveRail(strokeId = null, exitPoint = null) {
      ...
      if (rail) {
        ...
        const contact = exitPoint || (stroke ? this.getStrokePointAtDistance(stroke, rail.progress) : null) || rail.releaseExit || null;
        Body.setStatic(this.probe, false);
        if (contact) {
          Body.setVelocity(this.probe, {
    ...
    drawBackground(ctx, time) {
      ...
      for (const nebula of theme.nebulae) {
        ...
        ctx.rotate(nebula.angle + time * 2e-5);
      ...
      if (theme.galaxy) {
        ...
        ctx.rotate(theme.galaxy.angle + time * 4e-5);
        ctx.globalCompositeOperation = "lighter";
        ctx.shadowBlur = 50;
      ...
      for (const star of this.stars) {
        ...
        const x = (star.x * this.width + Math.sin(time * 8e-5 + star.phase) * star.drift * 18 + drift * 10 * star.z) % this.width;
        const y = (star.y * this.height + Math.cos(time * 5e-5 + star.phase) * star.drift * 9 + drift * 4 * star.z) % this.height;
      ...
      if (theme.meteors) {
        ...
        for (const meteor of theme.meteors) {
          const px = meteor.x + Math.cos(time * 12e-5 * meteor.speed + meteor.phase) * 18;
          const py = meteor.y + Math.sin(time * 8e-5 * meteor.speed + meteor.phase) * 10;
    ...
    drawHintPath(ctx, time) {
      ctx.save();
      ctx.globalCompositeOperation = "lighter";
      const phase = (time || performance.now()) * 8e-3;
    ...
    drawStartPortal(ctx, time) {
      const pulse = 0.65 + Math.sin(time * 2e-3) * 0.15;
      ctx.save();
    ...
    drawSpeedRings(ctx, time) {
      for (const ring of this.speedRings) {
        ...
        ctx.globalAlpha = 0.4 + Math.sin(time * 6e-3) * 0.14;
  ...
  function renderTutorialPage() {
    ...
    <div class="tutorial-pages">
      ${tutorialPages.map(
      (page2, index) => `
            <article class="tutorial-card tutorial-page tutorial-page--${page2.accent} ${index === tutorialIndex ? "is-active" : ""}">
              <div class="tutorial-card__index">0${index + 1}</div>
              <h3>${page2.title}</h3>
              <p>${page2.body}</p>
              <div class="tutorial-card__grid">
                ${buildTutorialVisual(page2.accent)}
              </div>
            </article>
          `
    ).join("")}
